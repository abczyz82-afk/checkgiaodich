...
SYMBOL_TIMEOUT = 30                  # Timeout (giây) cho mỗi mã — quá thì bỏ
...
def fetch_all_intraday(
    ...
    Mã nào chạy quá SYMBOL_TIMEOUT giây → bỏ qua thật sự.
    ...
    def _update_progress(current_sym: str = ""):
        if progress_bar is not None:
            progress_bar.progress(
                done / total,
                text=(
                    f"Đang tải: {done}/{total} mã  |  "
                    f"OK: {done - failed - timed_out}  |  "
                    f"Lỗi: {failed}  |  Timeout: {timed_out}  |  "
                    f"Mã: {current_sym}"
                ),
            )

    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        # Submit tất cả & ghi lại thời điểm submit
        future_map: dict = {}   # future → symbol
        submit_time: dict = {}  # future → timestamp
        for sym in symbols:
            f = executor.submit(fetch_intraday_single, sym)
            future_map[f] = sym
            submit_time[f] = time.time()

        pending = set(future_map.keys())

        while pending:
            # Chờ future tiếp theo hoàn thành (tối đa 2s rồi quét timeout)
            completed, pending = wait(
                pending, timeout=2.0, return_when=FIRST_COMPLETED
            )

            # Xử lý các future đã xong
            for future in completed:
                ...
                try:
                    ...
                    failed += 1
                _update_progress(sym)

            # Quét pending: mã nào đã chờ quá SYMBOL_TIMEOUT → bỏ
            now = time.time()
            expired = {
                f for f in pending
                if now - submit_time[f] > SYMBOL_TIMEOUT
            }
            for f in expired:
                sym = future_map[f]
                f.cancel()          # Hủy nếu chưa chạy
                pending.discard(f)
                done += 1
                timed_out += 1
    ...
    if status_text is not None:
        msg = (
            f"✅ Hoàn tất: {done - failed - timed_out}/{total} mã thành công, "
            f"{failed} lỗi, {timed_out} timeout (>{SYMBOL_TIMEOUT}s)."
        )
        if timed_out_symbols:
            msg += f"  ⏱️ Bỏ qua: {', '.join(timed_out_symbols[:20])}"
            if len(timed_out_symbols) > 20:
                msg += f" … và {len(timed_out_symbols) - 20} mã khác"
